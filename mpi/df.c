/*- * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  1. Utiliser des communications collectives
     a. pour la diffusion de la taille du tableau. FAIT
     b. pour la diffusion des valeurs du tableau.  FAIT
     c. pour le rassemblement des résulats. FAIT
     d. pour le rassemblement de l'erreur.  FAIT
...
main (int argc, char *argv[])
{
  ...
  /* diffusion de la taille de ce tableau aux autres */
	double start,end;
	start = MPI_Wtime ();
	MPI_Bcast(&lsize,1,MPI_INT,0,com);
  ...
  /* diffusion des valeurs du tableau */
    MPI_Scatter(a_io.val,lsize,MPI_FLOAT,a_local.val+1,lsize,MPI_FLOAT,0,com);
  /* mes voisins */
...
MPI_Gather(a_local.val+1,lsize,MPI_FLOAT,a_io.val,lsize,MPI_FLOAT,0,com);
	end =  MPI_Wtime ();
